...
xfail_if_old_configparser = pytest.mark.xfail(using_old_configparser, reason="configparser doesn't support empty_lines_in_values")


def _mock_calls_to_string(called_mock):
    return [
        "{}|{}|{}".format(name, args[0] if len(args) > 0 else args, repr(kwargs) if len(kwargs) > 0 else "") for name, args, kwargs in called_mock.mock_calls
    ]
...
""".format(
    ...
    log_capture.check_present(
        ...
        ("bumpversion.version_part", "INFO", "Parsing version '{}' using regexp '(?P<major>\\d+)\\.(?P<minor>\\d+)\\.(?P<patch>\\d+)'".format(version)),
        ("bumpversion.version_part", "INFO", "Parsed the following values: major={}, minor={}, patch={}".format(v_parts[0], v_parts[1], v_parts[2])),
        ("bumpversion.cli", "INFO", "Attempting to increment part 'patch'"),
        ("bumpversion.cli", "INFO", "Values are now: major={}, minor={}, patch={}".format(p_parts[0], p_parts[1], p_parts[2])),
        ("bumpversion.cli", "INFO", "Dry run active, won't touch any files."),  # only in dry-run mode
        ("bumpversion.version_part", "INFO", "Parsing version '{}' using regexp '(?P<major>\\d+)\\.(?P<minor>\\d+)\\.(?P<patch>\\d+)'".format(patch)),
        ("bumpversion.version_part", "INFO", "Parsed the following values: major={}, minor={}, patch={}".format(p_parts[0], p_parts[1], p_parts[2])),
        ...
        (
            "bumpversion.cli",
            "INFO",
            "Would tag 'v{p}' with message 'Bump version: {v} → {p}' in {vcs} and not signing".format(v=version, p=patch, vcs=vcs_name),
        ),
        order_matters=True,
    )
